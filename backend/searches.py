...
# Function to search UAS sightings by time range and proximity
    return db.query(models.UASSighting).filter(
        and_(
            models.UASSighting.time >= start_time,
...
# Function to search UAS sightings by proximity
    all_sightings = db.query(models.UASSighting).all()
    nearby = []
    for sighting in all_sightings:
        dist = haversine(latitude, longitude, sighting.latitude, sighting.longitude)
        if dist <= radius_km:
